...
class UpdateSupportUserScopeRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        return [
            ...
            'visibility_mode' => ['sometimes', 'required', Rule::in(['All', 'Team', 'Assigned', 'TeamAndAssigned', 'Customer', 'Own'])],
            'team_ids' => ['sometimes', 'nullable', 'array', 'max:200'],
            'team_ids.*' => ['required_with:team_ids', 'string', 'max:120', 'distinct'],
            'queue_ids' => ['sometimes', 'nullable', 'array', 'max:200'],
    ...
    protected function prepareForValidation(): void
    {
        $this->merge([
            ...
            'visibility_mode' => $this->has('visibility_mode')
                ? $this->input('visibility_mode')
                : $this->input('ticket_visibility', $this->input('ticketVisibility')),
            'team_ids' => $this->has('team_ids') ? $this->normalizeArray($this->input('team_ids')) : $this->normalizeArray($this->input('teamIds', [])),
            'queue_ids' => $this->has('queue_ids') ? $this->normalizeArray($this->input('queue_ids')) : $this->normalizeArray($this->input('queueIds', [])),
            'customer_ids' => $this->has('customer_ids') ? $this->normalizeArray($this->input('customer_ids')) : $this->normalizeArray($this->input('customerIds', [])),
